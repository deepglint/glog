...
func init() {
	...
	flag.StringVar(&logging.table, "table_name", "libra", "table name for database")
...
func (l *loggingT) header(s severity) *buffer {
	...
	//////////////////////////////////////////////////////////
	if !logging.debug {
		buf.WriteString(logging.table)

		measurementkv := ",program=" + program
	...
	if l.toStderr {
		...
		WRITE_FILE:
			...
			if err != nil && tries < 10 {
				...
				defer syscall.Flock(int(l.file[infoLog].file.Fd()), syscall.LOCK_UN)
				l.file[infoLog].file.Write(data)
			}
		} else {
			if l.file[s] == nil {
				if err := l.createDebugFiles(s); err != nil {
					os.Stderr.Write(data) // Make sure the message appears somewhere.
					l.exit(err)
				}
			}
...
RETRY:
	err := syscall.Flock(int(file.file.Fd()), syscall.LOCK_EX|syscall.LOCK_NB)
	if err == nil {
		file.Flush() // ignore error
		file.Sync()  // ignore error
		defer syscall.Flock(int(file.file.Fd()), syscall.LOCK_UN)
